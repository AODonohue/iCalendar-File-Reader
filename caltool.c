 ...
 * PURPOSE: Command line interface to process ics ver. 2 files.
 *********************************************************************/
 ...
 * PURPOSE: Prints an error message to stderr saying the error in more 
 *          readable terms, based on the status.
 * ARGUMENTS: The name of the file that was parsed, and the status it 
              received.
 *********************************************************************/
 ...
 * PURPOSE: Frees any allocated memory within a CalEvent.
 * ARGUMENTS: The CalEvent to free the allocated memory in.
 *********************************************************************/
 ...
 * PURPOSE: Compare function used to qsort CalEvents by date.
 * ARGUMENTS: Two void (CalEvent) pointers to compare.
 * RETURNS: 1 if first > second, -1 if first < second, or 0 if equal.
 *********************************************************************/
static int compDates(const void *date1, const void *date2) {
 ...
 * PURPOSE: Compare function used to qsort strings alphabetically.
 * ARGUMENTS: Two void (char*) pointers to compare.
 * RETURNS: 1 if first > second, -1 if first < second, or 0 if equal.
 *********************************************************************/
...
/*********************************************************************
 * FUNCTION NAME: numComps
 * PURPOSE: Counts top level V components.
 * ARGUMENTS: The CalComp to sum up the V components.
 * RETURNS: The number of top level V compenents.
 *********************************************************************/
static int numComps(const CalComp *comp) {
    int comps = 0;
...
/*********************************************************************
 * FUNCTION NAME: numEvents
 * PURPOSE: Counts how many VEVENTs are in a component.
 * ARGUMENTS: The component to sum up the VEVENTs.
 * RETURNS: The number of VEVENTs.
 *********************************************************************/
static int numEvents(const CalComp *comp) {
    int events = 0;
...
/*********************************************************************
 * FUNCTION NAME: numTodos
 * PURPOSE: Counts how many VTODOs are in a component.
 * ARGUMENTS: The component to sum up the VTODOs.
 * RETURNS: The number of VTODOs.
 *********************************************************************/
static int numTodos(const CalComp *comp) {
    int todos = 0;
...
/*********************************************************************
 * FUNCTION NAME: numSubComps
 * PURPOSE: Counts how many subcomponents are in a component.
 * ARGUMENTS: The component to sum up the subcomponents.
 * RETURNS: The number of subcomponents.
 *********************************************************************/
static int numSubComps(const CalComp *comp) {
    int subcomps = 0;
    ...
    for(int i = 0; i < comp->ncomps; i++)
        subcomps += comp->comp[i]->ncomps + numSubComps(comp->comp[i]);
...
/*********************************************************************
 * FUNCTION NAME: numOrgs
 * PURPOSE: Counts how many organizers are in a component. Stores the 
 *          organizers CN value into orgList.
 * ARGUMENTS: The component to sum up the organizers, the adress of 
 *            the array of strings to store the organizers in, and 
 *            the number of previously found organizers.
 * RETURNS: The number of organizers.
 *********************************************************************/
static int numOrgs(const CalComp *comp, char ***orgList, int organs) {
    for(int i = 0; i < comp->ncomps; i++) {
            ...
            if(strcmp(prop->name, "ORGANIZER") == 0) {
                for(int k = 0; k < prop->nparams; k++) {
                    if(strcmp(param->name, "CN") == 0) {
                        *orgList = realloc(*orgList, (organs + param->nvalues + 1) *sizeof(char*));
                        assert(*orgList != NULL);
        ...
        organs = numOrgs(comp->comp[i], orgList, organs);
    }
    return organs;
}
...
/*********************************************************************
 * FUNCTION NAME: numProps
 * PURPOSE: Counts how many properties are in a component.
 * ARGUMENTS: The component to sum up the properties.
 * RETURNS: The number of properties.
 *********************************************************************/
static int numProps(const CalComp *comp) {
    int props = 0;
 ...
 * PURPOSE: Checks if a property contains a time value.
 * ARGUMENTS: The property to check if it's a time property.
 ...
 * PURPOSE: Creates a string that is easier to parse in strptime, than
 *          the property value.
 * ARGUMENTS: The string to match a date with in an easier way.
...
/*********************************************************************
 * FUNCTION NAME: getFirstDate
 * PURPOSE: Fetches the earliest date from a component tree.
 * ARGUMENTS: The component to retreive the oldest date from.
...
/*********************************************************************
 * FUNCTION NAME: getLastDate
 * PURPOSE: Fetches the latest date from a component tree.
 * ARGUMENTS: The component to retreive the newest date from.
 * RETURNS: The string of the date in format (yyyymmddThhmmss[Z]).
 *********************************************************************/
 ...
 * PURPOSE: Checks of a component lies within two dates. If dates are
 *          unspecified, any time is good.
 * ARGUMENTS: The component to check, and the two dates to see if the 
 *            component is between them.
 * RETURNS: True if the component falls within the dates, false 
 *          otherwise.
 *********************************************************************/
...
/*********************************************************************
 * FUNCTION NAME: singlify
 * PURPOSE: Nullifies strings in a list that appear more than once.
 * ARGUMENTS: The array of strings to remove duplicates from, and how 
 *            many elements it has.
 * RETURNS: How many non NULL elements remain in the list.
 *********************************************************************/
static int singlify(char **list, int elements) {
    int removed = 0;
 ...
 * PURPOSE: Creates an array of strings that are all the X properties
 *          in a component, and stores how many elements are in the 
 *          array in xprops.
 * ARGUMENTS: The component to list the xproperties of, the adress of 
 *            the array of strings to store their names in, and the 
 *            adress of how many elements are being stored in the 
 *            array.
 *********************************************************************/
...
CalStatus calInfo(const CalComp *comp, int lines, FILE *const txtfile) {
    ...
    char *from     = "No dates";
    char *to       = "";
    int components = numComps(comp);
    int events     = numEvents(comp);
    int todos      = numTodos(comp);
    int others     = (components - events) - todos;
    int subcomps   = numSubComps(comp);
    int organizers = numOrgs(comp, &orgList, 0);
    int props      = numProps(comp);
    int nonDupes   = 0;
    int linesWrote = 0;
    struct tm date;
    ...
    qsort(orgList, organizers, sizeof(char*), compStr);
    nonDupes = singlify(orgList, organizers);
...
CalStatus calExtract(const CalComp *comp, CalOpt kind, FILE *const txtfile) {
    ...
    if(kind == OEVENT) {
        ...
        qsort(event, events, sizeof(CalEvent), compDates);
        ...
        qsort(xprop, xprops, sizeof(char*), compStr);
        singlify(xprop, xprops);
